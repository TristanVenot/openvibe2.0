; define a default for dep source directory
!ifndef DEPENDENCIES_DIR
	!define DEPENDENCIES_DIR "..\..\dependencies_x64"
!endif

	!define OV_VERSION "2.2.0"
	!define OV_VERSION_SHORT "220"
	
!ifndef OUTFILE
	!define OUTFILE "openvibe-${OV_VERSION}-64bit-setup.exe"
!endif
	...
	;Name and file
	!define OV_NAME "OpenViBE ${OV_VERSION} (64bit)"
	Name "${OV_NAME}"
	OutFile ${OUTFILE}

	;To detect a previous installation
	!define OV_REGKEY "openvibe${OV_VERSION_SHORT}-64bit"
	
	;Default installation folder
	InstallDir "$PROGRAMFILES64\openvibe-${OV_VERSION}-64bit"
	Var OLDINSTDIR
	Var DIRECTX_MISSING
...
no_need_to_install_directx:
	...
	NSISdl::download "https://download.microsoft.com/download/9/3/F/93FCF1E7-E6A4-478B-96E7-D4B285925B00/vc_redist.x64.exe" "arch\vc_redist.x64.exe"
	Pop $R0 ; Get the return value
		StrCmp $R0 "success" +3
			...
			Quit
	ExecWait '"arch\vc_redist.x64.exe" /install /norestart /passive'
no_need_to_install_vc140:
	...
	SetOutPath "$INSTDIR"
	; Export binaries
	File /nonfatal /r ..\..\dist\extras-Release-x64\bin
	; Export launch scripts
	File /nonfatal ..\..\dist\extras-Release-x64\*.cmd
	; File /nonfatal /r ..\dist\doc
	; File /nonfatal /r ..\dist\include
	
	; etc and lib folders are needed for the gtk theme
	File /nonfatal /r /x *.lib ..\..\dist\extras-Release-x64\lib
	File /nonfatal /r ..\..\dist\extras-Release-x64\etc
	
	File /nonfatal /r ..\..\dist\extras-Release-x64\log
	File /nonfatal /r ..\..\dist\extras-Release-x64\share
...
no_need_to_patch_3d_functionnality:
	...
	CreateShortCut "$SMPROGRAMS\${OV_NAME}\OpenViBE Designer 64bit.lnk"                           "$INSTDIR\openvibe-designer.cmd"            "" "%SystemRoot%\system32\shell32.dll" 137
	CreateShortCut "$SMPROGRAMS\${OV_NAME}\OpenViBE Acquisition Server 64bit.lnk"                 "$INSTDIR\openvibe-acquisition-server.cmd"  "" "%SystemRoot%\system32\shell32.dll" 18
	CreateShortCut "$SMPROGRAMS\${OV_NAME}\OpenViBE Tracker 64bit.lnk"                            "$INSTDIR\openvibe-tracker.cmd"  "" "%SystemRoot%\system32\shell32.dll" 249
	CreateShortCut "$SMPROGRAMS\${OV_NAME}\OpenViBE VR-demo Spaceship 64bit.lnk"                  "$INSTDIR\openvibe-vr-demo-spaceship.cmd"   "" "%SystemRoot%\system32\shell32.dll" 200
