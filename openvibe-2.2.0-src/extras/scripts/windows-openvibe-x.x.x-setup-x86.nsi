...
!ifndef OUTFILE
	!define OUTFILE "openvibe-${OV_VERSION}-32bit-setup.exe"
!endif
	...
	;Name and file
	!define OV_NAME "OpenViBE ${OV_VERSION} (32bit)"
	Name "${OV_NAME}"
	OutFile ${OUTFILE}
...
no_need_to_install_directx:
	...
	NSISdl::download "https://download.microsoft.com/download/9/3/F/93FCF1E7-E6A4-478B-96E7-D4B285925B00/vc_redist.x86.exe" "arch\vc_redist.x86.exe"
	Pop $R0 ; Get the return value
		StrCmp $R0 "success" +3
			...
			Quit
	ExecWait '"arch\vc_redist.x86.exe" /install /norestart /passive'
no_need_to_install_vc140:
	...
	SetOutPath "$INSTDIR"
	; Export binaries
	File /nonfatal /r ..\..\dist\extras-Release-x86\bin
	; Export launch scripts
	File /nonfatal ..\..\dist\extras-Release-x86\*.cmd
	; File /nonfatal /r ..\dist\doc
	; File /nonfatal /r ..\dist\include
	
	; etc and lib folders are needed for the gtk theme
	File /nonfatal /r /x *.lib ..\..\dist\extras-Release-x86\lib
	File /nonfatal /r ..\..\dist\extras-Release-x86\etc
	
	File /nonfatal /r ..\..\dist\extras-Release-x86\log
	File /nonfatal /r ..\..\dist\extras-Release-x86\share
...
no_need_to_patch_3d_functionnality:
	...
	CreateShortCut "$SMPROGRAMS\${OV_NAME}\OpenViBE Designer 32bit.lnk"                           "$INSTDIR\openvibe-designer.cmd"            "" "%SystemRoot%\system32\shell32.dll" 137
	CreateShortCut "$SMPROGRAMS\${OV_NAME}\OpenViBE Acquisition Server 32bit.lnk"                 "$INSTDIR\openvibe-acquisition-server.cmd"  "" "%SystemRoot%\system32\shell32.dll" 18
	CreateShortCut "$SMPROGRAMS\${OV_NAME}\OpenViBE Tracker 32bit.lnk"                            "$INSTDIR\openvibe-tracker.cmd"  "" "%SystemRoot%\system32\shell32.dll" 249
	CreateShortCut "$SMPROGRAMS\${OV_NAME}\OpenViBE VR-demo Spaceship 32bit.lnk"                  "$INSTDIR\openvibe-vr-demo-spaceship.cmd"   "" "%SystemRoot%\system32\shell32.dll" 200
