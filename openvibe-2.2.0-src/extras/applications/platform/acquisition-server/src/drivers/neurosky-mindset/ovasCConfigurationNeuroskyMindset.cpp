...
CConfigurationNeuroskyMindset::CConfigurationNeuroskyMindset(IDriverContext& rDriverContext, const char* sGtkBuilderFileName,uint32& rComPort,boolean& rESenseChannels,boolean& rBandPowerChannels,boolean& rBlinkStimulations,boolean& rBlinkStrengthChannel)
	...
	,m_rBlinkStrengthChannel(rBlinkStrengthChannel)
{
}
...
boolean CConfigurationNeuroskyMindset::preConfigure(void)
{
	...
	*/
	...
	int32 l_iDllVersion = TG_GetVersion();
	...
	::GtkToggleButton* l_pToggleBlinkStrength = GTK_TOGGLE_BUTTON(gtk_builder_get_object(m_pBuilderConfigureInterface, "check_blink_strenght"));
	...
	gtk_toggle_button_set_active(l_pToggleBlinkStrength,m_rBlinkStrengthChannel);
...
boolean CConfigurationNeuroskyMindset::postConfigure(void)
{
	if(m_bApplyConfiguration)
	{
		...
		m_rBlinkStrengthChannel = (::gtk_toggle_button_get_active(l_pToggleBlinkStrength) != 0);
	}
