	...
	,m_ui32WarningCount(-1)
{
	...
	m_oSettings.add("StrengthChannel", &m_bBlinkStrengthChannel);
	m_oSettings.load();

#if !defined(TG_DATA_BLINK_STRENGTH)
	m_rDriverContext.getLogManager() << LogLevel_Trace << "Mindset Driver: This version does not have eyeblink detection support\n";
	m_bBlinkStimulations = false;
	m_bBlinkStrengthChannel = false;
#endif
...
{
	return "NeuroSky MindSet (MindSet Dev. Tools 3.2)";
}
...
#if defined(TG_DATA_BLINK_STRENGTH)					
	if(m_bBlinkStimulations || m_bBlinkStrengthChannel)
	{
