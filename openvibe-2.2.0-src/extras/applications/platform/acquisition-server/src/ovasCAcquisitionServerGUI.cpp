...
//	l_fpReleaseMensiaAcquisitionLibrary();
	...
	m_pAcquisitionServerThread= nullptr;

	delete m_pAcquisitionServer;
	m_pAcquisitionServer= nullptr;
}
...
boolean CAcquisitionServerGUI::initialize(void)
{
	...
	gtk_spin_button_update(l_pSpinButtonConnectionPort);
	...
	{
		GtkWidget* mainWindow = GTK_WIDGET(gtk_builder_get_object(m_pBuilderInterface, "openvibe-acquisition-server"));
...
{
	GtkSpinButton* button = GTK_SPIN_BUTTON(::gtk_builder_get_object(m_pBuilderInterface, "spinbutton_connection_port"));
		...
		case GTK_RESPONSE_YES:
			...
			for (size_t setting_index = 0; setting_index < m_vPluginProperties.size(); ++setting_index)
			{
				...
				else if( TypedProperty<int64>* r = dynamic_cast< TypedProperty<int64>* >(l_pCurrentProperty))
				{
					...
					gtk_spin_button_update(button);
					int64 tmp = static_cast<int64>(::gtk_spin_button_get_value(button));
					r->replaceData( tmp );
				} 
				else if( TypedProperty<uint32>* r = dynamic_cast< TypedProperty<uint32>* >(l_pCurrentProperty))
				{
					...
					GtkSpinButton* button = GTK_SPIN_BUTTON(m_vPluginProperties[setting_index].m_pWidget);
					gtk_spin_button_update(button);
					uint32 tmp = static_cast<uint32>(::gtk_spin_button_get_value(button));
					r->replaceData( tmp );
				} 
				else if( TypedProperty<float32>* r = dynamic_cast< TypedProperty<float32>* >(l_pCurrentProperty)) 
				{
					...
					GtkSpinButton* button = GTK_SPIN_BUTTON(m_vPluginProperties[setting_index].m_pWidget);
					gtk_spin_button_update(button);
					float32 tmp = static_cast<float32>(::gtk_spin_button_get_value(button));
					r->replaceData( tmp );
				} 
				else 
				{
