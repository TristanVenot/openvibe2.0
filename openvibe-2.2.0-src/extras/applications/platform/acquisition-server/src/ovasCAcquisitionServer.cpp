...
namespace OpenViBEAcquisitionServer
{
	...
	public:
		...
		void operator()(void)
		{
			Socket::IConnection* l_pConnection= nullptr;
			do
			{
...
boolean CAcquisitionServer::connect(IDriver& rDriver, IHeader& rHeaderCopy, uint32 ui32SamplingCountPerSentBlock, uint32 ui32ConnectionPort)
{
	...
	if(testClient->connect("localhost", ui32ConnectionPort, 100))  // 100ms
	{
		m_rKernelContext.getLogManager() << LogLevel_Error << "Server Connection Port " << ui32ConnectionPort << " already in use. Please change the port or close the other application.\n";
		testClient->close();
		delete testClient;
		return false;
	}
	delete testClient; testClient = nullptr;
...
boolean CAcquisitionServer::start(void)
{
	...
	for(auto itp = m_vPlugins.begin(); itp != m_vPlugins.end(); ++itp)
	{
		bool ok = (*itp)->startHook(m_vSelectedChannelNames, m_ui32SamplingFrequency, m_ui32ChannelCount, m_ui32SampleCountPerSentBlock);
		if(!ok)
		{
			m_rKernelContext.getLogManager() << LogLevel_Error << "Problem starting a plugin, bailing out for safety.\n";
			return false;
		}
	}
...
boolean CAcquisitionServer::disconnect(void)
{
	...
	if(m_pConnectionServerHandlerStdThread)
	{
		...
		m_pConnectionServerHandlerStdThread= nullptr;
	}
...
boolean CAcquisitionServer::acceptNewConnection(Socket::IConnection* pConnection)
{
	...
	l_oInfo.m_pConnectionClientHandlerThread= nullptr; // not used
	l_oInfo.m_pConnectionClientHandlerStdThread= nullptr; // not used
	l_oInfo.m_bChannelLocalisationSent = false;
	l_oInfo.m_bChannelUnitsSent = false;
