...
bool CBoxAlgorithmMatlabScripting::OpenMatlabEngineSafely(void)
{
	...
	__except(EXCEPTION_EXECUTE_HANDLER)
	{
		...
		this->getLogManager() << LogLevel_Error << "To use this box you must have MATLAB installed on your computer.\n";
		this->getLogManager() << LogLevel_Error << "For this build of OpenViBE, MATLAB needs to be a " <<
...
bool CBoxAlgorithmMatlabScripting::initialize(void)
{
	...
	for(uint32_t i=0; i<getStaticBoxContext().getInputCount(); i++)
	{
		...
		TDecoder<CBoxAlgorithmMatlabScripting> * l_pDecoder = nullptr;
	...
	for(uint32_t i=0; i<getStaticBoxContext().getOutputCount(); i++)
	{
		...
		TEncoder<CBoxAlgorithmMatlabScripting> * l_pEncoder = nullptr;
...
bool CBoxAlgorithmMatlabScripting::process(void)
{
	...
	m_oMatlabHelper->setManagers(&this->getLogManager(), &this->getErrorManager());
