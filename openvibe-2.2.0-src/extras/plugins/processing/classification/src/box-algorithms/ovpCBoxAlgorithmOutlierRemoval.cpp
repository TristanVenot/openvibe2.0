...
boolean CBoxAlgorithmOutlierRemoval::process(void)
{
	...
	for(uint32 i=0; i<l_rDynamicBoxContext.getInputChunkCount(1); i++)
	{
		...
		if(m_oFeatureVectorDecoder.isBufferReceived()) 
		{
			...
			l_oFeatureVector.m_ui64StartTime=l_rDynamicBoxContext.getInputChunkStartTime(1, i);
			l_oFeatureVector.m_ui64EndTime=l_rDynamicBoxContext.getInputChunkEndTime(1, i);
