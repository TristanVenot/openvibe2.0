	...
	,m_ui64SentSampleCount(0)
{
	...
	std::default_random_engine l_oRandomDevice(System::Math::randomUInteger32());
	m_oGenerator.seed(l_oRandomDevice());
}
...
boolean CNoiseGenerator::initialize(void)
{
	...
	m_ui64NoiseType = FSettingValueAutoCast(*this->getBoxAlgorithmContext(), 3);
...
boolean CNoiseGenerator::process(void)
{
	...
	else
	{
		...
		const uint64 l_ui64BufferEntries = m_oSignalEncoder.getInputMatrix()->getBufferElementCount();
		if(m_ui64NoiseType == OVP_TypeId_NoiseType_Uniform)
		{
			for(uint64_t i=0;i<l_ui64BufferEntries;i++)
			{
				l_pSampleBuffer[i]=static_cast<float64>(m_oUniformDistribution(m_oGenerator));
			}
		...
		else
		{
			for(uint64_t i=0;i<l_ui64BufferEntries;i++)
			{
				l_pSampleBuffer[i]=static_cast<float64>(m_oNormalDistribution(m_oGenerator));
			}
		}
