		...
		namespace SimpleVisualization
	{
		...
		class Graph
		{
			...
			public:
				...
				void updateCurves(const OpenViBE::float64* curve, size_t howMany, unsigned int curveIndex);
				void snapCoords(cairo_t * cairoContext, double* x, double* y);
				OpenViBE::float64 adjustValueToScale(OpenViBE::float64 value);

				std::vector <std::vector<OpenViBE::float64> > m_vCurves; //private
				std::vector< std::vector<OpenViBE::float64> > m_vVariance;

				std::vector< ::GdkColor>& m_cLineColor; //private
				std::vector<OpenViBE::CString>& m_sLineText; //private
				OpenViBE::float64 m_pMaximum = 1;
				OpenViBE::float64 m_pMinimum = -1;
		...
		public:
			...
			virtual OpenViBE::Plugins::IPluginObject* create(void)       { return new OpenViBEPlugins::SimpleVisualization::CBoxAlgorithmErpPlot; }
